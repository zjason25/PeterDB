...
//            printf("reading page: %d, slotnum: %d\n", rids[i].pageNum, rids[i].slotNum);
//            std::ostringstream stream1;
//            std::ostringstream stream2;
//            rbfm.printRecord(recordDescriptor, outBuffer, stream1);
...
//            printf("inserting record %d", i);
        ...
        outBuffer = malloc(1000);
//        printf("Examining records");
        for (int i = 0; i < numRecords; i++) {
//            printf("Examining record %d", i);
            memset(inBuffer, 0, 1000);
            memset(outBuffer, 0, 1000);
            ASSERT_EQ(rbfm.readRecord(fileHandle, recordDescriptor, rids[i], outBuffer), success)
                                        << "Reading a record should succeed.";
